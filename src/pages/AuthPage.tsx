...
export default function AuthPage({ mode }: { mode?: "signup" | "login" } = {}) {
  ...
  const resolvedMode = mode || (searchParams.get("signup") === "true" || location.pathname === "/cadastro" ? "signup" : "login");
  const [isSignup, setIsSignup] = useState(resolvedMode === "signup");
  ...
  // Sincroniza isSignup se o usuário navegar entre /cadastro e /login
  useEffect(() => {
    const newMode = mode || (new URLSearchParams(location.search).get("signup") === "true" || location.pathname === "/cadastro" ? "signup" : "login");
